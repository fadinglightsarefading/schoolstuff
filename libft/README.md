...
- `ft_striteri()` does the same thing, except it doesn't return anything new; it just modifies the
...
- `ft_putnbr_fd()` takes an `int` and writes it as text in the file.
...
the function.
- `ft_lstclear()` applies `ft_lstdelone()` on the given node and all those posterior to it.
...
[^1]: `-Llibft` assumes the directory containing the static library is called `libft` and is
present in the same place where the programme to be compiled is; if the library is present
elsewhere, instead of writing `libft` after `-L`, write its path.
